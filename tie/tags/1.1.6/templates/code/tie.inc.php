...
class TIE
{
						...
						foreach ($results as $row)
						{
							...
							if (!(file_exists($filepath) && !isset($_POST['overwrite'])))
							{
								...
								if (!$error)
								{
									...
									$char = $this->breakConvert(PHP_OS); //Choose appropiate newline characters.
	...
	**/
	private function breakConvert($source)
	{
	...
	**/
	public function parse($output, $eval, $open, $close, $escape = false)
	{
		$return = array();
		$output = strval($output);
	...
	public function replacePositions($array, $output)
	{
		if (is_array($array))
		{
			...
			foreach ($array as $key => $value)
			{
				...
				{
					$position = -1;
					while (($position = stripos($output, $value[0], $position+1)) !== false)
					{
						$pass = true;
