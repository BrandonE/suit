...
class TIE
{
	...
	public function adminArea($path, $redirect, $post, $type)
	{
		...
		else
		{
				...
				foreach ($templates as $value)
				{
					if ($iterations >= $this->navigation->settings['start'])
					{
						...
						(
							...
							array('<search>', $this->navigation->settings['search']),
							array('<start>', $this->navigation->settings['start']),
							array('<title>', htmlspecialchars(basename($value, '.' . $filetype)))
						);
					}
					$iterations++;
			...
			if (!empty($templates))
			{
				foreach ($templates as $value)
				{
					$return = $this->parseConditional(htmlspecialchars(basename($value, '.' . $filetype)), ($this->navigation->settings['select']), $return);
				}
			}
		}
	...
	**/	
	...
	public function parseConditional($string, $bool, $output)
	{
		...
		preg_match_all('/\<if\s' . $string . '\>(.*?)\<\/if\s' . $string . '\>/msx', $output, $parse, PREG_SET_ORDER);
		foreach ($parse as $key => $value)
		{
			$replacement = ($bool) ? $parse[$key][1] : '';
			$array[] = array($parse[$key][0], $replacement);
		}
		preg_match_all('/\<else\s' . $string . '\>(.*?)\<\/else\s' . $string . '\>/msx', $output, $parse, PREG_SET_ORDER);
		foreach ($parse as $key => $value)
		{
			$replacement = ($bool) ? '' : $parse[$key][1];
			$array[] = array($parse[$key][0], $replacement);
	...
	public function parseLoop($string, $replace, $output, $implode = '')
	{
		...
		$array = array();
		$string = str_replace(' ', '\s', $string);
		preg_match_all('/\<loop\s' . $string . '\>(.*?)\<\/loop\s' . $string . '\>/msx', $output, $parse, PREG_SET_ORDER);
		foreach ($parse as $key => $value)
		{
			$return = array();
			foreach ($replace as $value2)
			{
				$return[] = $this->replace($parse[$key][1], $value2);
			}
			$array[] = array($parse[$key][0], implode($implode, $return));
		}
		$output = $this->replace($output, $array);
		return $output;
	}
