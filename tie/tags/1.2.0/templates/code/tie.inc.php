...
class TIE
{
	...
	**/
	...
	{
		$return = array();
		$parse = $this->parseMulti($string, $content);
	...
	**/
	...
	{
		$return = array();
		$parse = $this->parseMulti($if, $content);
		foreach ($parse as $value)
		{
		...
		if ($else !== false)
		{
			$parse = $this->parseMulti($else, $content);
			foreach ($parse as $value)
			{
	...
	**/
	...
	/**
	Matches Conditions for Multi-Rule Parse
	**@param string Name
	**@param string Content
	**@returns array Matches
	**/
	private function parseMulti($string, $content)
	{
		$return = array();
		$content = strval($content);
		...
		preg_match_all('/' . $open . $string . $close . '((?:(?!' . $open . $string . $close . ').)*?)' . $open . $end . $string . $close . '/msx', $content, $return, PREG_SET_ORDER);
		return $return;
	}
