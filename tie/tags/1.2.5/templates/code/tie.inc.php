...
class TIE
{
	...
	**/
	public function adminArea($type, $admin_delete = false, $admin_form = false, $admin_list = false, $admin_xml = false, $badrequest = false)
	{
		...
		$redirect = $this->navigation->path($_SERVER['SCRIPT_NAME'], array('boxes', 'cmd', 'file', 'select'));
		...
		if ($type != 'code')
		{
			if (isset($_POST['import']) && isset($_FILES['file']))
			{
				...
				if ($_FILES['file']['type'] == 'text/xml')
				{
					...
					foreach ($results as $row)
					{
						...
						{
							$title = $this->replace($illegal, $title[0]); //Format the filename.
					...
					if (!$error)
						...
						foreach ($results as $row)
						{
							...
							$title = (!empty($title)) ?
								str_replace('/', '', str_replace('\\', '', $title[0])) :
								'';
							...
							if ($type == 'glue')
							{
								...
								while (isset($content[strlen($content) - ($count + 1)]) && ($content[strlen($content) - ($count + 1)] == '\\'))
									$count++;
								$content = (!empty($content)) ?
									str_replace('=', '\=', $content) . str_repeat('\\', $count) :
									'';
							}
							$filepath = $this->owner->templates . '/' . $type . '/' . $title . '.' . $filetype;
									...
									if ($type == 'glue')
										foreach ($code as $key => $value)
										{
											...
											while (isset($value[strlen($value) - ($count + 1)]) && ($value[strlen($value) - ($count + 1)] == '\\'))
												$count++;
											$code[$key] = str_replace('=', '\=', $value) . str_repeat('\\', $count);
										}
									$content = ($type == 'glue') ?
										$content .
										(
										...
										$this->replace($linebreak, $content);
			...
			elseif ((isset($_GET['cmd']) && ($_GET['cmd'] == 'export')) || isset($_POST['exportselected']) && isset($_POST['entry']) && (is_array($_POST['entry'])))
			{
				...
				foreach ($id as $value)
				{
					$filepath = $this->owner->templates . '/' . $type . '/' . $this->replace($illegal, $value) . '.' . $filetype;
					if (file_exists($filepath))
					{
						if ($type == 'glue')
						{
							...
							foreach ($array as $value2)
								$code[] = array
								(
									array('<codetoken>', $this->replace($codearray, $value2))
								);
						}
						else
						{
						...
						(
							...
							(
								array
								(
									array('<contenttoken>', $this->replace($contentarray, $content)),
									array('<titletoken>', $this->replace($titlearray, $value))
								),
			...
			elseif (isset($_POST['deleteselected']) && isset($_POST['entry']) && is_array($_POST['entry']))
			{
				...
				$this->navigation->redirect($redirect . '&cmd=delete&file[]=' . $get, '', 0);
			}
		...
		if (isset($_GET['cmd']) && ((in_array($_GET['cmd'], array('add', 'delete', 'edit')) && $type != 'code') || ($_GET['cmd'] == 'view' && $type == 'code')))
		{
			...
			{
				$title = $this->replace($illegal, $posted['title']); //Make sure the filename has no illegal characters.
				$posted['oldtitle'] = $this->replace($illegal, $posted['oldtitle']);
					...
					if (!$error)
						if (is_writable($filepath2))
						{
							...
							$posted['content'] = ($type == 'glue') ?
								...
								$this->replace($linebreak, $posted['content']);
		...
		else
		{
			...
			if (is_array($templates))
				foreach ($templates as $key => $value)
				{
					...
					if ($value == '.' || $value == '..' || $value == (basename($value, '.' . $filetype)) || $pos === false)
						unset($templates[$key]);
				}
				...
				foreach ($templates as $value)
				{
					if ($iterations >= $this->navigation->settings['start'])
					{
						...
						(
							...
							$this->parseConditional('if entries_checked', ($this->navigation->settings['select']), $return), //Is this submission's checkbox ticked?
							array
							(
		...
		if (isset($redirectmessage) && ($redirectmessage))
		{
			$templates = scandir($this->owner->templates . '/' . $type);
			foreach ($templates as $key => $value)
			...
			if (count($templates) <= $this->navigation->settings['start'])
			{
				...
				$redirect = $this->navigation->path($_SERVER['SCRIPT_NAME'], array('boxes', 'cmd', 'file', 'select', 'start'));
	...
	**/
	public function replace($array, $return)
	{
		if (is_array($array))
		{
			...
			if (array_key_exists(0, $array) && (array_key_exists(0, $array[0]) && (!is_array($array[0][0]))))
				$array = array($array);
	...
	**/
	...
	/**
	Strpos or Stripos based on config
	**@param string Haystack
	**@param string Needle
	**@param string Offset
	**@returns mixed Results of function call
	**/
