...
class TIE
{
	...
	public function adminArea($path, $redirect, $post, $type)
	{
				...
				if ($_FILES['file']['type'] == 'text/xml')
				{
					...
					if (!$error)
					{
						...
						foreach ($results as $row)
						{
							...
							$code = ($code) ? html_entity_decode($code[0]) : '';
							...
							if (!(file_exists($filepath) && !isset($_POST['overwrite'])))
							{
								...
								if (!$error)
								{
									...
									$char = $this->breakConvert($content, PHP_OS); //Choose appropiate newline characters.
									$content = ($type == 'glue') ? $this->fileFormat($content) . '=' . $this->fileFormat($code) : preg_replace('/(\\r\\n)|\\r|\\n/', $char, $content); //If it is a glue, then format it for such. Otherwise, we convert the newline style for it.
		...
		if (isset($_GET['cmd']) && (in_array($_GET['cmd'], $pages)) && $type != 'code')
		{
			...
			if (!isset($redirectmessage))
			{
				...
				elseif (!strcmp($_GET['cmd'], 'delete'))
				{
					...
					$return = $this->replace($this->parseConditional('section delimeter', false, $return), $return);
		...
		else
		{
			...
			if (!empty($templates))
			{
				...
				$return = $this->replace($this->parseConditional('section highlightstart', false, $return), $return);
				$return = $this->replace($this->parseConditional('section highlightend', false, $return), $return);
				foreach ($templates as $value)
				{
	...
	**/
	public function getSection($string, $output)
	{
		...
		$return = array();
		$parse = array();
		preg_match_all('/\<' . $string . '\>((?:(?!\<' . $string . '\>).)*?)\<\/' . $string . '\>/msx', $output, $parse, PREG_SET_ORDER);
	...
	**/
	public function parseConditional($if, $bool, $output, $else = false)
	{
		...
		preg_match_all('/\<' . $if . '\>((?:(?!\<' . $if . '\>).)*?)\<\/' . $if . '\>/msx', $output, $parse, PREG_SET_ORDER);
		foreach ($parse as $value)
		{
		...
		{
			$else = strval($else);
			$else = preg_quote($else, '/');
			//Escape whitespace with the whitespace character(\s)
			$else = str_replace(' ', '\s', $else);
			$else = str_replace('	', '\s', $else);
			//Remove Linebreaks(\n) and Carriage Returns (\r), replacing them with whitespace(\s)
			$else = preg_replace('/(\\r\\n)|\\r|\\n/', '\s', $else);
			//Match <else expression_here>Condition</else expression_here> as ELSE conditionals.
			preg_match_all('/\<' . $else . '\>((?:(?!\<' . $else . '\>).)*?)\<\/' . $else . '\>/msx', $output, $parse, PREG_SET_ORDER);
			foreach ($parse as $value)
			{
