...
if ($suit->loggedIn() == 2)
{
	$layered[$layer+1] = $suit->templates->getDynamicTemplate('admin_indexcontentskeleton', $layer+1);
	$lcontent = $suit->language->getLanguage('adminwelcome');
	$layered[$layer+1] = str_replace('{1}', $lcontent, $layered[$layer+1]);
	$layered[$layer+2] = $suit->templates->getDynamicTemplate('admin_notes', $layer+2);
	
	$notes = $suit->mysql->select('notes', '*');
	if ($notes)
	{
