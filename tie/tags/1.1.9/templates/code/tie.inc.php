...
class TIE
{
	...
	**/
	public function __construct(&$owner, $config)
	{
		...
		if (isset($this->config['cookie']['domain']) && isset($this->config['cookie']['length']) && isset($this->config['cookie']['path']) && isset($this->config['cookie']['prefix']))
		{
			...
			$languages = $this->owner->vars['languages'];
	...
	**/
	public function adminArea($type)
	{
		...
		if ($type != 'code')
		{
			...
			{
				$id = (isset($_GET['cmd']) && !strcmp($_GET['cmd'], 'export')) ?
					$_GET['file'] :
					$_POST['entry'];
				...
				$xml = $this->owner->getTemplate($this->config['templates']['admin_xml']);
				...
				foreach ($id as $value)
				{
					...
					if (file_exists($filepath))
					{
						if ($type == 'glue')
						{
							...
							$array = preg_split('/(?<!\\\)\=/msx', file_get_contents($filepath), -1, PREG_SET_ORDER);
				...
				if (!$templates)
					$this->owner->getTemplate($this->config['templates']['badrequest']); //None of the selected templates exist.
			...
			elseif (isset($_POST['deleteselected']) && isset($_POST['entry']) && is_array($_POST['entry']))
			{
				...
				$this->navigation->redirect(0, $path . 'start=' . $this->navigation->settings['start'] . '&limit=' . $this->navigation->settings['limit'] . '&order=' . $this->navigation->settings['order'] . '&search=' . $this->navigation->settings['search'] . '&cmd=delete&file[]=' . $get, '');
			}
		...
		if (isset($_GET['cmd']) && ((in_array($_GET['cmd'], array('add', 'delete', 'edit')) && $type != 'code') || ($_GET['cmd'] == 'view' && $type == 'code')))
		{
			if ((isset($_POST['add']) && $status != 2) || (isset($_POST['edit']) || isset($_POST['editandcontinue'])) && ($status == 0))
			{
				...
				if (isset($_POST['add']))
					...
					if (file_exists($filepath))
						...
						$this->owner->getTemplate($this->config['templates']['badrequest']);
				if (!$error)
				{
					$error = $this->checkWritable($title, $type, $filetype);
					if (!$error)
						if (is_writable($filepath2))
						{
							if ($type == 'glue')
								foreach ($posted['code'] as $key => $value)
									$posted['code'][$key] = str_replace('=', '\=', $value);
							$posted['content'] = ($type == 'glue') ?
								str_replace('=', '\=', $posted['content']) .
								(
									(!empty($posted['code'])) ?
										'=' . implode('=', $posted['code']) :
										''
									...
									if (is_array($templates))
										foreach ($templates as $value)
											if ($value != '.' && $value != '..' && $value != (basename($value, '.txt')) && !(is_array($value)))
											{
												...
												$array = preg_split('/(?<!\\\)\=/msx', file_get_contents($this->owner->templates . '/glue/' . $value), -1, PREG_SET_ORDER);
						...
						if (isset($_POST['glue']) && $type == 'content')
						{
							...
							foreach ($templates as $value2)
								if ($value2 != '.' && $value2 != '..' && $value2 != (basename($value, '.txt')))
								{
									...
									$array = preg_split('/(?<!\\\)\=/msx', file_get_contents($this->owner->templates . '/glue/' . $value2), -1, PREG_SET_ORDER);
			...
			if (!isset($redirectmessage))
			{
				...
				if ((!strcmp($_GET['cmd'], 'edit') || !strcmp($_GET['cmd'], 'delete')) && !isset($_GET['file']))
					$this->owner->getTemplate($this->config['templates']['badrequest']);
				else
					$filepath = $this->owner->templates . '/' . $type . '/' . str_replace('/', '', str_replace('\\', '', $_GET['file'])) . '.' . $filetype;
				//Nonexistant file specified.
				if (!is_array($_GET['file']) && !file_exists($filepath))
					$this->owner->getTemplate($this->config['templates']['badrequest']);
				...
				$return = $this->owner->getTemplate((!strcmp($_GET['cmd'], 'delete')) ?
				...
				(
					...
					$this->parseConditional('if editing', (!strcmp($_GET['cmd'], 'edit')), $return)
				);
				$return = $this->replace($array, $return);
				if (!strcmp($_GET['cmd'], 'delete'))
				{
					...
					$return = $this->replace($this->parseLoop('if glue', false, $return), $return);
					$array = array
					(
						array('<error>', $error),
						array('<name>', 'delete'),
				...
				{
					$posted['title'] = (isset($_GET['file'])) ?
						$_GET['file'] :
						'';
					if ($type == 'glue')
					{
						$array = preg_split('/(?<!\\\)\=/msx', (file_exists($filepath)) ? file_get_contents($filepath) : '', -1, PREG_SET_ORDER);
						$posted['content'] = (isset($array[0])) ?
							str_replace('\=', '=', $array[0]) :
							'';
						unset($array[0]);
						$posted['code']  = $array;
						...
						$posted['content'] = (isset($filepath) && file_exists($filepath)) ?
							file_get_contents($filepath) :
							'';
					$posted['code'] = (!isset($posted['code']) || empty($posted['code'])) ?
						array('') :
						$posted['code'];
					if ($type == 'glue')
					{
						...
						foreach ($posted['code'] as $value)
						{
							...
							(
								array
								(
									array('<code>', htmlentities(str_replace('\=', '=', $value))),
									array('<number>', $number)
								)
							);
							$number++;
					...
					(
						...
						(
							...
							array('<name>',  ((!strcmp($_GET['cmd'], 'add')) ? 'add' : 'edit')),
							array('<title>', htmlentities($posted['title'])),
							array('<value>', ((!strcmp($_GET['cmd'], 'add')) ? $this->language['add'] : $this->language['edit']))
						),
		...
		{
			$return = $this->owner->getTemplate($this->config['templates']['admin_list']);
	...
	**/
	...
	{
		$url = strval($url);
		return file_get_contents($url . ((strstr($url, '?')) ? '&' : '?') . 'template=' . $template);
	}
	...
	**/
	public function getSection($string, $content)
	{
		...
		$return = array();
		$parse = array();
		preg_match_all('/' . $open . $string . $close . '((?:(?!' . $open . $string . $close . ').)*?)' . $open . $end . $string . $close . '/msx', $content, $parse, PREG_SET_ORDER);
	...
	**/
	public function parse($eval, $open, $close, $content, $escape = false)
	{
		...
		preg_match_all('/' . $open . '((?:(?!' . $open . ').)*?)' . $close . '/msx', $content, $return, PREG_SET_ORDER);
		$temp = $return;
		if ($escape !== false)
		{
			$escape = strval($escape);
			$section = $this->getSection($escape, $content);
			...
			$positions = $this->replacePositions($return, $content);
			$return = $positions[1];
		}
	...
	**/
	public function parseConditional($if, $bool, $content, $else = false)
	{
		$content = strval($content);
		$if = strval($if);
		...
		preg_match_all('/' . $open . $if . $close . '((?:(?!' . $open . $if . $close . ').)*?)' . $open . $end . $if . $close . '/msx', $content, $parse, PREG_SET_ORDER);
		foreach ($parse as $value)
		{
		...
		if ($else !== false)
		{
			...
			preg_match_all('/' . $open . $else . $close . '((?:(?!' . $open . $else . $close . ').)*?)' . $open . $end . $else . $close . '/msx', $content, $parse, PREG_SET_ORDER);
			foreach ($parse as $value)
			{
	...
	**/
	public function parseLoop($string, $replace, $content, $implode = '')
	{
		$content = strval($content);
	...
	**/
	private function replacePositions($array, $content)
	{
		if (is_array($array))
		{
			$content = strval($content);
			...
			foreach ($array as $key => $value)
			{
				...
				{
					$position = -1;
					while (($position = stripos($content, strval($value[0]), $position+1)) !== false)
					{
						$pass = true;
