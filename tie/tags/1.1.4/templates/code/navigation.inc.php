	...
	private function pageLink($count, $check, $start, $display, $ahead)
	{
		...
		if ($success)
		{
			...
			(
				...
				$this->owner->parseConditional('current', false, $pagelink)
			);
	...
	**/
	public function pagination($count, $pages = 2)
	{
		...
		(
			...
			$this->owner->parseConditional('current', true, $return['current'])
		);
	...
	**/
	public function redirect($refresh, $url, $message = '')
	{
		...
		if ($message != '')
		{
			...
			$seconds = $this->owner->replace($this->owner->parseConditional('s', ($refresh != 1), $seconds), $seconds);
			$array = array
			(
