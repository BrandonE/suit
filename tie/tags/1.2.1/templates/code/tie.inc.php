...
class TIE
{
	...
	**/
	...
	{
		$return = array();
		$parse = $this->parseMatch($this->config['parse']['sections']['open'] . $string . $this->config['parse']['sections']['close'], $this->config['parse']['sections']['open'] . $this->config['parse']['sections']['end'] . $string . $this->config['parse']['sections']['close'], $content);
	...
	**/
	public function parse($eval, $open, $close, &$content)
	{
		$return = array();
		...
		$return = $this->parseMatch($open, $close, $content);
	...
	**/
	...
	{
		$return = array();
		$parse = $this->parseMatch($this->config['parse']['sections']['open'] . $if . $this->config['parse']['sections']['close'], $this->config['parse']['sections']['open'] . $this->config['parse']['sections']['end'] . $if . $this->config['parse']['sections']['close'], $content);
		foreach ($parse as $value)
		{
		...
		if ($else !== false)
		{
			$parse = $this->parseMatch($this->config['parse']['sections']['open'] . $else . $this->config['parse']['sections']['close'], $this->config['parse']['sections']['open'] . $this->config['parse']['sections']['end'] . $else . $this->config['parse']['sections']['close'], $content);
			foreach ($parse as $value)
			{
	...
	**/
	private function parseEscape($pos, $escape, $content)
	{
		$replace = false;
		$count = 0;
		while (isset($content[$pos - ($count + 1)]) && ($content[$pos - ($count + 1)] == $this->config['parse']['escape']))
			$count++;
		$originalcount = $count;
		$condition = $count % 2;
		if ($condition)
			$count++;
		if ($count)
			$replace = array(str_repeat($this->config['parse']['escape'], $originalcount) . $escape, str_repeat('\\', $originalcount - ($count / 2)) . $escape);
	...
	**/
	private function parseMatch($open, $close, &$content)
	{
		$return = array();
		$array = array();
		$open = strval($open);
		...
		while (($pos = (($this->config['flag']['insensitive']) ? stripos($content, $open, $pos + 1) : strpos($content, $open, $pos + 1))) !== false)
		{
			...
			{
				$call = $this->parseEscape($openpos, $open, $content);
				if ($call[1])
					$escape[] = $call[1];
			}
			$call[0] = true;
			...
			{
				$call = $this->parseEscape($closepos, $close, $content);
				if ($call[1])
					$escape[] = $call[1];
			}
			$call = $this->parseEscape($pos, $open, $content);
				if ($call[1])
					$array[] = $call[1];
