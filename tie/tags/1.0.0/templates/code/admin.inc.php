	...
	elseif (isset($_POST['import']) && isset($_FILES['file']))
	{
		...
		if (class_exists('SimpleXMLElement'))
		{
			...
			if ($_FILES['file']['type'] == 'text/xml')
			{
				...
				{
					if (isset($row->title) && isset($row->content) && isset($row->code))
					{
						$filepath = $suit->checkFile($row->title);
						if (!$filepath || ($_POST['overwrite'] && is_writable($filepath['content']) && is_writable($filepath['code'])))
						{
							$filepath = $suit->checkFile($row->title, true);
							//OS specific linebreak conversions to avoid linebreaking issues.
							$char = $suit->tie->breakConvert($row->code, PHP_OS);
							$content = preg_replace('/(\\r\\n)|\\r|\\n/', $char, $row->content);
							file_put_contents($filepath['content'], $content); //Write the contents to the file.
							$code = preg_replace('/(\\r\\n)|\\r|\\n/', $char, $row->code);
							file_put_contents($filepath['code'], $code); //Write the contents to the file.
						}
					}
				}
		...
		{
			$error = '<p>' . $suit->tie->language['simplexmlfail'] . '</p>';
		}
	}
...
elseif (isset($_GET['cmd']) && (in_array($_GET['cmd'], $pages)))
{
	...
	if (isset($_POST['add']) && $status != 2)
	{
		...
		if (!$error)
		{
			$filepath = $suit->checkFile($posted['title'], true);
			$char = $suit->tie->breakConvert($posted['code'], PHP_OS);
			$posted['content'] = preg_replace('/(\\r\\n)|\\r|\\n/', $char, $posted['content']);
			file_put_contents($filepath['content'], $posted['content']);
			$posted['code'] = preg_replace('/(\\r\\n)|\\r|\\n/', $char, $posted['code']);
			file_put_contents($filepath['code'], $posted['code']);
	...
	elseif (isset($_POST['edit']) && !strcmp($status, 0))
	{
		...
		if ($filepath)
		{
			...
			if (!$filepath2 || $posted['title'] == $posted['oldtitle'])
			{
				if (!strcmp($posted['title'], ''))
				{
					$error = $suit->tie->language['missingtitle'];
	...
	elseif (isset($_POST['delete']) && isset($posted['title']))
	{
		foreach ($posted['title'] as $value)
		{
			...
			if ($filepath)
			{
				if (!(is_writable($filepath['content']) && is_writable($filepath['code'])))
				{
					$error = $suit->tie->language['notchmod'];
				}
			}
