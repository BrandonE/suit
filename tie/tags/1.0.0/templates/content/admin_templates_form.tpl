<error>
		<form action="#" method="post">
		<input type="hidden" name="oldtitle" value="<oldtitle>" />
		[inputtitle]: <input type="text" name="title" value="<title>" />
		<br />[content]: <textarea name="content" rows="40" cols="100" wrap="off" style="width: 100%;" class="textarea"><content></textarea>
		<br /><input type="submit" name="escape" value="[escape]" tabindex="1" />
		<br /><br />[phpcode]:
		<br /><textarea name="code" rows="40" cols="100" wrap="off" style="width: 100%;" class="textarea"><code></textarea>
		<br /><input type="submit" name="<name>" value="<value>" tabindex="0" />
		</form>