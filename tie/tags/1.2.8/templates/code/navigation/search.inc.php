...
if (isset($_POST['navigation_search_submit']) && isset($_POST['navigation_search_value']))
	$suit->tie->navigation->redirect($path . 'start=0&limit=' . $suit->tie->navigation->settings['limit'] . '&order=' . $suit->tie->navigation->settings['order'] . '&search=' . urlencode($_POST['navigation_search_value']) . '&check=' . $checked, false, 0);
