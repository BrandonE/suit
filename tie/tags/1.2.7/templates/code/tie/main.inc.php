	...
	**/
	public function parseMatch($open, $close, &$content, $escapestring = false)
	{
		...
		while (($pos = $this->strpos($content, $open, $pos + 1)) !== false)
		{
			$call = array();
			$escape = array();
			$call = $this->parseUnescape($pos, $content, $escapestring);
			$pos -= ($call[1] / 2);
			$content = substr_replace($content, '', $pos, ($call[1] / 2));
			if (!$call[0])
			{
				$openpos = $pos;
				$closepos = $pos;
				...
				{
					$call = $this->parseUnescape($closepos, $content, $escapestring);
					$closepos -= ($call[1] / 2);
					$content = substr_replace($content, '', $closepos, ($call[1] / 2));
				}
				$call[0] = true;
				while ($call[0] && ($openpos = $this->strpos($content, $open, $openpos + 1)) !== false && ($openpos < $closepos))
				{
					$call = $this->parseUnescape($openpos, $content, $escapestring);
					$openpos -= ($call[1] / 2);
					$closepos -= ($call[1] / 2);
					$content = substr_replace($content, '', $openpos, ($call[1] / 2));
				}
	...
	private function parseUnescape($pos, $content, $escapestring)
	{
		$replace = array();
		$pos = intval($pos);
		$content = strval($content);
						...
						if (!in_array($value2[0], $taken))
						{
							...
							while (($position = $this->strpos($return, $value2[0], $position + 1)) !== false)
							{
								...
								for ($x = 0; $x < strlen(strval($value2[0])); $x++)
									if (in_array($position + $x, $positions))
									{
										$pass = false;
										break;
									}
