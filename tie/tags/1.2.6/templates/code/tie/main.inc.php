...
class TIE
{
	...
	**/
	public function adminArea($type, $tie_delete = false, $tie_form = false, $tie_list = false, $tie_xml = false, $badrequest = false)
	{
												...
												if (is_writable($filepath2))
												{
													if ($type == 'glue' && !isset($_POST['rename']) && !isset($_POST['copy']) && !isset($_POST['move']))
													{
														if (!empty($value['code']))
															...
															{
																$parseescape = $this->parseEscape($value2);
																$value['code'][$key] = str_replace('=', '\=', $parseescape[0]) . str_repeat('\\', $parseescape[1]);
															}
														$parseescape = $this->parseEscape($value['content']);
														$value['content'] = str_replace('=', '\=', $parseescape[0]) . str_repeat('\\', $parseescape[1]) .
														(
															(!empty($value['code'])) ?
																'=' . implode('=', $value['code']) :
																''
				...
				elseif ((isset($_GET['cmd']) && ($_GET['cmd'] == 'export')) || isset($_POST['exportchecked']))
				{
					...
					foreach ($directories as $value)
					{
						...
						foreach ($dir as $value2)
						{
							...
							$array[] = array
							(
								array('<arraytoken>', $this->replace($directoryarrayescape, $parseescape[0]) . str_repeat('\\', $parseescape[1]))
							);
						}
						$parseescape = $this->parseEscape(basename($value));
						$directoriesarray[] = array_merge
						(
							array
							(
								array('<titletoken>', $this->replace($directorytitleescape, $parseescape[0]) . str_repeat('\\', $parseescape[1]))
							),
					...
					foreach ($files as $value)
					{
						if ($type == 'glue')
						{
							...
							foreach ($array as $value2)
							{
								...
								$code[] = array
								(
									array('<codetoken>', $this->replace($filecodeescape, $parseescape[0]) . str_repeat('\\', $parseescape[1]))
								);
							}
						}
						else
						...
						foreach ($dir as $value2)
						{
							...
							$array[] = array
							(
								array('<arraytoken>', $this->replace($filearrayescape, $parseescape[0]) . str_repeat('\\', $parseescape[1]))
							);
						}
						$contentescape = $this->parseEscape($content);
						$titleescape = $this->parseEscape(basename($value, '.' . $filetype));
						$filesarray[] = array_merge
						(
							array
							(
								array('<contenttoken>', $this->replace($filecontentescape, $contentescape[0]) . str_repeat('\\', $contentescape[1])),
								array('<titletoken>', $this->replace($filetitleescape, $titleescape[0]) . str_repeat('\\', $titleescape[0]))
							),
							$this->parseLoop('loop array', $array, $xml),
		...
		if (isset($_GET['cmd']) && ((in_array($_GET['cmd'], array('add', 'copy', 'create', 'delete', 'edit', 'remove', 'rename')) && $type != 'code') || ($_GET['cmd'] == 'view' && $type == 'code')))
		{
			...
			if ((in_array($_GET['cmd'], array('edit', 'view')) && !is_file($filepath)) || ($_GET['cmd'] == 'rename' && in_array($title, array('.', '..'))))
				$this->owner->getTemplate($badrequest);
			$array = array
			(
				array_merge
		...
		else
		{
			...
			$page = (!empty($page)) ?
				$page[0] :
				'';
			$section = $this->language['page'] . $page . ($this->navigation->settings['start'] / $this->navigation->settings['limit'] + 1);
			$templates = array_diff(scandir($this->owner->templates . '/' . $type . $directory['string']), array('.', '..'));
			$files = array();
			$directories = array();
	...
	**/
	public function getSection($string, &$content, $open = false, $close = false, $end = false, $escapestring = false)
	{
		...
		$escapestring = strval(($escapestring !== false) ?
			$escapestring :
		...
		foreach ($parse[0] as $value)
			$return[] = $value[1];
		$content = $this->replace($parse[1], $content);
		return $return;
	}
	...
	**/
	public function parse($eval, $open, $close, $content, $escapestring = false)
	{
		...
		{
			$case = $value[1];
			$return[0][$key][1] = eval('return' . $eval . ';');
		}
		$return = array_merge
		(
			$return[0],
	...
	**/
	...
	public function parseEscape($content)
	{
		...
		while (isset($content[strlen($content) - ($count + 1)]) && ($content[strlen($content) - ($count + 1)] == '\\'))
			$count++;
		$content = str_replace('\\', '\\\\', substr($content, 0, strlen($content) - $count)) . substr($content, strlen($content) - $count);
	...
	**/
	public function parseLoop($string, $replace, $content, $implode = '', $open = false, $close = false, $end = false, $escapestring = false)
	{
		...
		if (is_array($replace) && (array_key_exists(0, $replace) && is_array($replace[0]) && ((array_key_exists(0, $replace[0]) && (is_array($replace[0][0]))) || empty($replace[0]))) || empty($replace))
		{
			$parse = $this->parseMatch($open . $string . $close, $open . $end . $string . $close, $content, $escapestring);
			foreach ($parse[0] as $value)
			{
	...
	**/
	public function parseMatch($open, $close, $content, $escapestring = false)
	{
		...
		while (($pos = $this->strpos($content, $open, $pos + 1)) !== false)
		{
			...
			$closepos = $pos;
			$call[0] = true;
			while ($call[0] && ($openpos = $this->strpos($content, $open, $openpos + 1)) !== false)
			{
				$call = $this->parseUnescape($openpos, $open, $content, $escapestring);
				if ($call[1])
					$escape[] = $call[1];
			}
			$call[0] = true;
			...
			{
				$call = $this->parseUnescape($closepos, $close, $content, $escapestring);
				if ($call[1])
					$escape[] = $call[1];
			}
			$end = ($call[2] != 0) ?
				$call[2] / 2 :
				0;
			$call = $this->parseUnescape($pos, $open, $content, $escapestring);
				if ($call[1])
					$donotparse[] = $call[1];
			if ((($closepos !== false && $openpos === false) || ($openpos > $closepos)) && !$call[0])
			{
				$match = substr($content, $pos + strlen($open), $closepos - ($pos + strlen($open)));
				$newmatch = $this->replace($escape, $match);
				$parse[] = array($open . $match . $close, substr($newmatch, 0, strlen($newmatch) - $end));
			}
		}
		return array($parse, $donotparse);
	}

	private function parseUnescape($pos, $escape, $content, $escapestring)
	{
		$replace = array();
		$pos = intval($pos);
		...
		return array($condition, $replace, $count);
	}
	...
	**/
	public function replace($array, $return)
	{
		...
		$array = array_values($array);
		if (is_array($array) && (array_key_exists(0, $array) && (is_array($array[0]))) || empty($array))
		{
