...
if (isset($_POST['navigation_search_submit']) && isset($_POST['navigation_search_value']))
	$suit->tie->redirect($path[0] . $path[2] . 'start=0&limit=' . $suit->tie->settings['limit'] . '&order=' . $suit->tie->settings['order'] . '&search=' . urlencode($_POST['navigation_search_value']) . '&check=' . $checked, false, 0);
