...
if ($tie->loggedIn() == 0)
{
	if (isset($_POST['lostpassword']))
	{
		...
		{	
			$usercheck_options = array('where' => 'email = \'' . $suit->db->escape($tie->magic($_POST['email']), 0) . '\'');
			...
			if ($usercheck)
			{
				while ($row = mysql_fetch_assoc($usercheck))
				{
					...
					mysql_query($query);
					$body = $tie->getPhrase('message_body');
					$body = str_replace('<password>', $password, $body);
					$body = str_replace('<PATH_URL>', PATH_URL, $body);
					$body = str_replace('<string>', $string, $body);
					$body = str_replace('<id>', $row['id'], $body);
					mail($row['email'], $tie->getPhrase('message_subject'), $body, $tie->getPhrase('emailheaders')) or die ($tie->getPhrase('maildeliveryfailed'));
				}
	...
	{
		$usercheck_options = array('where' => 'id = \'' . intval($_GET['id']) . '\' AND recover_string = \'' . $suit->db->escape($tie->magic($_GET['string']), 0) . '\'');
		$usercheck = $suit->db->select(DB_PREFIX . 'users', '*', $usercheck_options);
		if ($usercheck)
		{
			while ($row = mysql_fetch_assoc($usercheck))
