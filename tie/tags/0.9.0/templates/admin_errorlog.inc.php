...
if ($tie->loggedIn() == 2)
{
	...
	if ($search)
	{
		$options['where'] = 'MATCH (content, date, location) AGAINST (\'' . $search . '\')';
	}
	$list = $tie->createList(DB_PREFIX . 'errorlog', $options, $suit->templates->getTemplate('path_url') . '/index.php?page=admin_errorlog', 'id', 0, 0, 1, $start, $limit, 0, $search);
