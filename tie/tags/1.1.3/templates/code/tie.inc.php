		...
		if ($type != 'code')
		{
			...
			elseif ((isset($_GET['cmd']) && ($_GET['cmd'] == 'export')) || (isset($_POST['exportselected']) && is_array($_POST['entry'])))
			{
				...
				do
				{
					$array = array_merge
					(
						$this->parseConditional('code', ($type == 'glue'), $xml), //If the type of file requested was a glue file, then add the code entry to it.
						$this->parseLoop('templates', $templates, $xml) //Loop through the templates array in order to create the proper formatting.
					);
			...
			if (!isset($redirectmessage))
			{
				if (!strcmp($_GET['cmd'], 'add'))
				{
					...
					do
					{
						$array = array_merge
						(
							$this->parseConditional('editing', (!strcmp($_GET['cmd'], 'edit')), $return), //Determine if we are editing in order for allow display of the 'Save and Continue' button.
