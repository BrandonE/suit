...
class TIE
{
	...
	public function adminArea($path, $redirect, $post, $type)
	{
		...
		if ($type != 'code')
		{
			...
			elseif ((isset($_GET['cmd']) && ($_GET['cmd'] == 'export')) || (isset($_POST['exportselected']) && is_array($_POST['entry'])))
			{
				...
				//Loop through the templates array in order to create the proper formatting.
				$xml = $this->parseLoop('templates', $templates, $this->owner->getTemplate('admin_xml'));
				//If the type of file requested was a glue file, then add the code entry to it.
		...
		if (isset($_GET['cmd']) && (in_array($_GET['cmd'], $pages)) && $type != 'code')
		{
			...
			if (!isset($redirectmessage))
			{
				...
				elseif (!strcmp($_GET['cmd'], 'edit'))
				{
					...
					$return = $this->parseConditional('editing', (!strcmp($_GET['cmd'], 'edit')), $return); //Determine if we are editing in order for allow display of the 'Save and Continue' button.
					if (!$error)
					{
						$posted['title'] = $_GET['file'];
						if ($type == 'glue')
				...
				elseif (!strcmp($_GET['cmd'], 'delete'))
				{
					...
					$message = $this->parseLoop('names', $input, $this->language['deleteconfirm'], ', ');
					$array = array
					(
						array('<error>', $error),
						array('<name>', 'delete'),
				...
				$return = $this->parseConditional('editing', (!strcmp($_GET['cmd'], 'edit')), $return); //Are we editing?
			}
		}
		...
		if (isset($redirectmessage))
		{
			...
			$this->navigation->redirect($this->config['redirect']['interval'], $redirect, $redirectmessage);
		}
		$return = $this->parseConditional('content', ($type == 'content'), $return); //Whether or not to show the content box.
		$return = $this->parseConditional('code', ($type == 'code'), $return); //Are we going to use the code box?
		$return = $this->parseConditional('glue', ($type == 'glue'), $return); //Are we doing anything with the glue?
		$return = $this->parseConditional('checked', ($this->navigation->settings['select']), $return); //Is this submission's checkbox ticked?
		$return = $this->parseConditional('error', ($error), $return); //Are there any errors at all?
	...
	public function parseConditional($string, $bool, $return)
	{
		...
		do
		{
			...
			foreach ($parse as $value)
			{
				...
				$array[] = array($value[0], $replacement);
			}
		...
		while(!empty($array));
		//Now return the content with the conditional parsed properly.
		return $return;
	}
	/**
	Parses conditional statements in templates
	...
	public function parseLoop($string, $replace, $return, $implode = '')
	{
		...
		do
		{
			...
			foreach ($parse as $value)
			{
				...
				$array[] = array($value[0], implode($implode, $replacements));
			}
			$return = $this->replace($array, $return);
		}
		while(!empty($array));
	...
	**/
	public function parseLanguages($return)
	{
		$parse = array();
		...
		foreach ($parse as $key => $value)
		{
			...
			$parse[$key][1] = $this->language[$value[1]];
		}
	...
	**/
	public function parseTemplates($return)
	{
		...
		foreach ($parse as $key => $value)
		{
			...
			$parse[$key][1] = $this->owner->getTemplate($value[1]);
		}
		$return = $this->replace($parse, $return);
		return $return;
	}
