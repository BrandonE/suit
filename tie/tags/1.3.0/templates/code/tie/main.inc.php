	...
	**/
	public function adminArea($type, $badrequest = NULL, $delete = NULL, $form = NULL, $list = NULL, $xml = NULL)
	{
			...
			if ($error === false)
			{
				...
				)
				{
						...
						foreach ($_POST['entry'] as $value)
							if (is_writable($this->owner->templates . '/' . $type . $directory['string']))
							{
								if ($value != '')
								{
									...
									(
										'code' => array(),
										'content' => file_get_contents($this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped . '.' . $filetype),
					...
					elseif (isset($_POST['replace']) && is_array($_POST['entry']))
						foreach ($_POST['entry'] as $value)
							if ($value != '')
							{
								...
								if ($type == 'glue')
								{
									$glue = $this->owner->explodeUnescape('=', '\\', file_get_contents($this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped . '.' . $filetype));
									foreach ($glue as $key2 => $value2)
										$glue[$key2] = str_replace($_POST['find'], $_POST['replacewith'], $value2);
									$contenttoken = implode('=', $glue);
								}
								else
									$contenttoken = str_replace($_POST['find'], $_POST['replacewith'], file_get_contents($this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped . '.' . $filetype));
								$files[] = array
								(
									'code' => array(),
						...
						(
							...
							foreach ($title as $value)
								if ($value != $_POST['moveto'] || !isset($_POST['move']))
								{
									...
									{
										$stripped = $this->owner->replace($illegal, $value);
										if (!is_dir($this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped))
											$this->owner->getTemplate($badrequest);
										$templates = array_diff($this->rscandir($this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped . '/'), array('.', '..'));
										$templates = array_merge
										(
											array($this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped),
											$templates
										);
										foreach ($templates as $value2)
										{
											...
											if (is_writable($check))
											{
												...
												$string = (isset($_POST['move']) && ($_POST['moveto'] == '..')) ?
													$newdirectory :
													$directory['string'];
												$new = str_replace($this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped, $this->owner->templates . '/' . $type . $string . $moveto . $showtitle, $value2);
					...
					if ($error === false)
					{
						foreach ($directories as $value)
						{
							...
							$filepath = $this->owner->templates . '/' . $type . $thisdirectory['string'] . '/' . $value['title'];
													...
													if ($type == 'glue' && !isset($_POST['rename']) && !isset($_POST['copy']) && !isset($_POST['move']) && !isset($_POST['replace']))
													{
														if (!empty($value['code']))
															foreach ($value['code'] as $key => $value2)
																$value['code'][$key] = $this->owner->escape('=', '\\', $value2, true);
														$value['content'] = $this->owner->escape('=', '\\', $value['content'], true) .
														(
															(!empty($value['code'])) ?
																'=' . implode('=', $value['code']) :
																''
				...
				)
				{
					...
					if (is_array($files))
						foreach ($files as $key => $value)
						{
							$files[$key] = $this->owner->templates . '/' . $type . $directory['string'] . '/' . $this->owner->replace($illegal, $value) . '.' . $filetype;
					...
					if (is_array($directories))
						foreach ($directories as $key => $value)
						{
							$directories[$key] = $this->owner->templates . '/' . $type . $directory['string'] . '/' . $this->owner->replace($illegal, $value);
					...
					foreach ($directories as $value)
					{
						...
						foreach ($dir as $value2)
							$array[] = array
							(
								array('[arraytoken]', $this->owner->escape($this->owner->escape('<array>', '\\',
								$this->owner->escape('<code>', '\\',
								$this->owner->escape('<content>', '\\',
								$this->owner->escape('<directory>', '\\',
								$this->owner->escape('<file>', '\\',
								$this->owner->escape('<title>', '\\',
								$this->owner->escape('</array>', '\\',
								$this->owner->escape('</code>', '\\',
								$this->owner->escape('</content>', '\\',
								$this->owner->escape('</directory>', '\\',
								$this->owner->escape('</file>', '\\',
								$this->owner->escape('</title>', '\\',$value2)))))))))))), true)),
								array()
							);
						$value = basename($value);
						$directoriesarray[] = array
						(
							array('[titletoken]', $this->owner->escape('<array>', '\\',
							$this->owner->escape('<code>', '\\',
							$this->owner->escape('<content>', '\\',
							$this->owner->escape('<directory>', '\\',
							$this->owner->escape('<file>', '\\',
							...
							$this->owner->escape('</directory>', '\\',
							$this->owner->escape('</file>', '\\',
							$this->owner->escape('</title>', '\\', $value))))))))))), true)),
							array
							(
					...
					foreach ($files as $value)
					{
						...
						if ($type == 'glue')
						{
							...
							foreach ($array as $value2)
								$code[] = array
								(
									array('[codetoken]', $this->owner->escape('<array>', '\\',
									$this->owner->escape('<code>', '\\',
									$this->owner->escape('<content>', '\\',
									$this->owner->escape('<directory>', '\\',
									$this->owner->escape('<file>', '\\',
						...
						foreach ($dir as $value2)
							$array[] = array
							(
								array('[arraytoken]', $this->owner->escape('<array>', '\\',
								$this->owner->escape('<code>', '\\',
								$this->owner->escape('<content>', '\\',
								$this->owner->escape('<directory>', '\\',
								$this->owner->escape('<file>', '\\',
						...
						(
							array
							(
								array('[contenttoken]', $this->owner->escape('<array>', '\\',
								$this->owner->escape('<code>', '\\',
								$this->owner->escape('<content>', '\\',
								$this->owner->escape('<directory>', '\\',
								$this->owner->escape('<file>', '\\',
								...
								$this->owner->escape('</directory>', '\\',
								$this->owner->escape('</file>', '\\',
								$this->owner->escape('</title>', '\\', $content))))))))))), true)),
								array('[titletoken]', $this->owner->escape('<array>', '\\',
								$this->owner->escape('<code>', '\\',
								$this->owner->escape('<content>', '\\',
								$this->owner->escape('<directory>', '\\',
								...
								$this->owner->escape('</directory>', '\\',
								$this->owner->escape('</file>', '\\',
								$this->owner->escape('</title>', '\\', $title))))))))))), true))
							),
							array
							(
								$this->owner->parseLoop('loop array', $array),
		...
		)
		{
			...
			$filepath = $this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped . '.' . $filetype;
			$filepath2 = $this->owner->templates . '/' . $type . $directory['string'] . '/' . $stripped;
			if
			(
				(
			...
			if ($_GET['cmd'] == 'delete')
			{
				...
				if (is_array($_GET['title']))
					foreach ($_GET['title'] as $value)
					{
						$filepath = $this->owner->templates . '/' . $type . $directory['string'] . '/' . $this->owner->replace($illegal, $value) . '.' . $filetype;
				...
				if (is_array($_GET['directorytitle']))
					...
					{
						$filepath = $this->owner->templates . '/' . $type . $directory['string'] . '/' . $this->owner->replace($illegal, $value);
		...
		else
		{
			...
			if (is_array($templates))
				foreach ($templates as $key => $value)
				{
					...
					if (is_file($filepath = $this->owner->templates . '/' . $type . $directory['string'] . '/' . $value) && $value != basename($value, '.' . $filetype))
					{
						$file = false;
						if ($type == 'glue')
						{
					...
					elseif (is_dir($this->owner->templates . '/' . $type . $directory['string'] . '/' . $value) && $pos !== false)
						$directories[] = $value;
				}
			natcasesort($files);
				...
				foreach ($templates as $value)
				{
					if ($iterations >= $this->settings['start'])
					{
						...
						(
							...
							array
							(
								$this->owner->parseConditional('if file', (is_file($this->owner->templates . '/' . $type . $directory['string'] . '/' . $value)), 'else file'),
	...
	public function checkWritable($template, $directory, $filetype)
	{
		...
		if (!is_file($this->owner->templates . '/' . $directory . '/' . $template . '.' . $filetype))
			...
			{
				@touch($this->owner->templates . '/' . $directory . '/' . $template . '.' . $filetype) or $return = $this->owner->vars['language']['filecouldnotbecreated'];
