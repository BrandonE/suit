...
class TIE
{
	...
	**/
				...
				)
				{
					...
					if ($error === false)
					{
						...
						if ($error === false)
						{
							...
							{
								$thisdirectory = $this->directorydata($value['directory']);
								$error = $this->checkWritableFolder($thisdirectory['string'] . '/' . $value['title'], $thisdirectory['string'], $type);
								if ($error !== false)
									break;
							}
							if ($error === false)
							{
								foreach ($files as $value)
								{
									...
									if (!isset($_POST['delete']))
									{
										...
										if ($error === false)
										{
											$error = $this->checkWritable($value['title'], $thisdirectory['string'], $type, $filetype);
											if ($error === false)
	...
	public function checkWritableFolder($folder, $directory, $type)
	{
		$return = false;
		$folder = strval($folder);
		$directory = strval($directory);
		if (!is_dir($this->owner->config['templates'][$type] . $folder))
			if (is_writable($this->owner->config['templates'][$type] . $directory))
			{
				mkdir($this->owner->config['templates'][$type] . $folder);
				chmod($this->owner->config['templates'][$type] . $folder, 0777);
			}
			else
				$return = $this->owner->vars['language']['directorynotchmod'];
		return $return;
