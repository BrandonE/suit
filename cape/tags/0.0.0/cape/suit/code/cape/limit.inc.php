...
if (isset($_POST['navigation_limit_submit']) && isset($_POST['navigation_limit_value']))
	$suit->tie->redirect((($suit->community->config['mod_rewrite']) ?
		$suit->community->config['mod_rewriteurl'] . '/' . $suit->vars['string'] . '/0/' . intval($_POST['navigation_limit_value']) . '/' . $suit->tie->settings['order'] . '/' . $checked . '/' . $suit->tie->settings['search'] . '/' :
		$path[0] . $path[2] . 'start=0&limit=' . intval($_POST['navigation_limit_value']) . '&order=' . $suit->tie->settings['order'] . '&search=' . $suit->tie->settings['search'] . '&check=' . $checked), false, 0);
