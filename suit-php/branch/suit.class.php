...
class SUIT
{
    ...
    public function defaultconfig($config)
    {
        ...
        `compile`
            bool - Whether or not to compile the templates to the server-side
            language.
    ...
    public function execute($rules, $string, $config = array())
    {
        ...
        */
        ...
        if ($config['compile'])
        {
            //exit(json_encode($this->cache['compile']));
        }
        if ($config['log'])
        {
    ...
    public function functions($params)
    {
        ...
        // If the config says not to compile, run the specified functions.
        if (!$params['config']['compile'])
        {
        ...
        // Else, write the function into the code.
        else
        {
            $params['string'] = '<?php $params[\'config\']=$config;' .
            '$params[\'rules\']=$rules;$params[\'string\']=\'\';';
            $dumped = json_encode($params['tree']);
            $hashkey = md5(json_encode($params['tree']));
            // If this tree hasn't been used yet, add it to the cache.
            if (!in_array($hashkey, $this->cache['compile']))
            {
                $this->cache['compile'][$hashkey] = $dumped;
            }
            $params['string'] .= '$params[\'tree\']=json_decode($cache[' .
            json_encode($hashkey) . '],true);foreach($rules[' . json_encode(
                $rule
            ) . '][\'functions\']as$value){if(array_key_exists(\'class\',$va' .
            'lue)){$params=$value[\'class\']->$value[\'function\']($params);' .
            '}else{$params=$value[\'function\']($params);}}echo$params[\'str' .
            'ing\'];?>';
        }
        return $params;
    }
