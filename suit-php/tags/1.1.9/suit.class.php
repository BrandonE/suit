	...
	**/
	public function getTemplate($template)
	{
		...
		if (!in_array($template, $this->chain))
			...
			{
				$array = preg_split('/(?<!\\\)\=/msx', file_get_contents($this->filepath), -1, PREG_SET_ORDER);
				//The first explode result element is the content file. If the key exists, continue.
				if (isset($array[0]))
				{
					//If the file exists, continue.
					if (file_exists($this->filepath = $this->templates . '/content/' . str_replace('../', '', str_replace('\=', '=', $array[0])) . '.tpl'))
				...
				foreach ($array as $value)
					//If the file exists, continue.
					if (file_exists($this->filepath = $this->templates . '/code/' . str_replace('../', '', str_replace('\=', '=', $value)) . '.inc.php'))
					{
