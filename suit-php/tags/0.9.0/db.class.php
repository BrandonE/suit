...
class Database
{
		...
		switch (DB_TYPE)
		{
			case 'mysql':
				...
				if ($conn)
				{
					...
					mysql_select_db($database, $this->connection) or die('hurrdurr');
				}
				else
				{
	...
	**/
	...
	/**
	Add data into a Database Table with the specified information/
	**@param string Database Table
	**@param array Database Fields
	**@param array Database Field Values
	...
	**/
	function escape($value, $wildcards = false) 
	{
		...
		switch (DB_TYPE)
		{
			...
			case 'mysql':
				return mysql_real_escape_string($value); break;
		}
	}
