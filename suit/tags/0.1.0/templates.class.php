				...
				if ($templatecheck)
				{
					while ($row2 = mysql_fetch_assoc($templatecheck))
					{
						if ($layer == 0)
						{
							...
							{
								$this->getTemplate($parse['templates'][$key][1], $id, $pass, 1, 1);
							...
							{
								$this->getDynamicTemplate($parse['dynamictemplates'][$key][1], $id, $pass, 1);
						...
						if ($row2['dynamic'] == 1)
						{
							...
							switch ($template)
							{
								case 'loginmessage':
									...
									{
										$system->language->getLanguage($GLOBALS['message'], $id, $pass);
								...
								case 'templateslist':
									...
									if (!isset($_GET['id']))
									{
										...
										if ($parentget)
										{
											...
											{
												$system->templates->getTemplate('selecttemplateset', $id, $pass, $layer+1, 0);
												...
												if ($adminthemes)
												{
													...
													{
														$system->templates->getTemplate('associatedthemes', $id, $pass, $layer+1, 0);
									...
									else
									{
										if (!isset($_GET['template']))
										{
											...
											*/
											$query = 'SELECT * FROM ' . $tbl_prefix . 'templates WHERE parent = \'' . $id . '\' order by title';
											$result = mysql_query($query);
											if ($result)
											{
												while ($row = mysql_fetch_assoc($result))
												{
													$system->templates->getTemplate('selecttemplate', $id, $pass, $layer+1, 0);
													$list .= $GLOBALS['layered'][$layer+1];
										...
										else
										{
											...
											if ($admintemplates)
											{
												...
												{
													$system->templates->getTemplate('edittemplate', $id, $pass, $layer+1, 0);
													$list .= $GLOBALS['layered'][$layer+1];
													$list = str_replace('{1}', $row['id'], $list);
