...
class SUIT
{
	...
	**/
	public function __construct($templates, $config, $suit = 'suit', $content = 'content')
  	{
		...
		elseif (!is_dir($this->templates) || !is_dir($this->templates . '/content') || !is_dir($this->templates . '/code') || !is_dir($this->templates . '/glue'))
			$this->error('Templates directory or it\'s required one of it\'s required subdirectories does not exist. See http://www.suitframework.com/docs/errors#error4');
	}
	...
	**/
	public function getTemplate($template)
	{
		...
		if (!in_array($template, $this->chain))
			if (is_file($this->filepath = $this->templates . '/glue/' . $template . '.txt'))
			{
				$array = $this->explodeUnescape('=', '\\', file_get_contents($this->filepath));
				$array[0] = strval($array[0]);
				if ($array[0] && is_file($this->filepath = $this->templates . '/content/' . str_replace('../', '', str_replace('..\\', '', $array[0])) . '.tpl'))
				{
				...
				{
					$value = strval($value);
					if ($value && is_file($this->filepath = $this->templates . '/code/' . str_replace('../', '', str_replace('..\\', '', $value)) . '.inc.php'))
					{
