	...
	public function __construct($templates, $suit = 'suit', $content = 'content')
  	{
		...
		if (phpversion() <= '4.4.9')
			$this->error('PHP Version must be greater than 4.4.9. See http://www.suitframework.com/docs/error1/');
		elseif (!is_dir($this->templates) || !is_dir($this->templates . '/content') || !is_dir($this->templates . '/code') || !is_dir($this->templates . '/glue'))
			$this->error('Templates directory or it\'s required one of it\'s required subdirectories does not exist. See http://www.suitframework.com/docs/error4/');
	}

	private function error($error, $plain = false, $type = 'Error')
	{
		$plain = strval(($plain !== false) ?
			$plain :
			$error);
...
<b>SUIT ' . $type . '</b>:  ' . $error . ' in <b>' . $backtrace[1]['file'] . '</b> on line <b>' . $backtrace[1]['line'] . '</b><br />';
			error_log('SUIT ' . $type . ':  ' . $plain . ' in ' . $backtrace[1]['file'] . ' on line ' . $backtrace[1]['line']);
		}
		if ($type == 'Error')
			exit;
	}
	...
	public function getTemplate($template)
	{
		...
		if (!in_array($template, $this->chain))
			if (file_exists($this->filepath = $this->templates . '/glue/' . $template . '.txt'))
			{
				...
				$this->error('The following template could not be found:<pre>' . htmlspecialchars($template) . '</pre>See http://www.suitframework.com/docs/error2/', 'The following template could not be found: "' . $template . '". See http://www.suitframework.com/docs/error2/', 'Warning');
		else
			$this->error('Infinite Loop Caused by <pre>' . htmlspecialchars($template) . '</pre>See http://www.suitframework.com/docs/error3/', 'Infinite Loop Caused by "' . $template . '". See http://www.suitframework.com/docs/error3/', 'Warning');
		return $return;
	}
